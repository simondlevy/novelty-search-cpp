...
bool Genome::mutate_add_node(std::vector<Innovation*> &innovs,int &curnode_id,double &curinnov) {
    ...
    while(!done) {
        ...
        else if (((*theinnov)->innovation_type==Innovation::NEWNODE)&&
...
bool Genome::mutate_add_link(std::vector<Innovation*> &innovs,double &curinnov,int tries) {
    ...
    if (found) {
        ...
        while(!done) {
            ...
            else if (((*theinnov)->innovation_type==Innovation::NEWLINK)&&
...
void Genome::mutate_add_sensor(std::vector<Innovation*> &innovs,double &curinnov) {
    ...
    unsigned int outputConnections = 0;
...
Genome *Genome::mate_multipoint(Genome *g,int genomeid,double fitness1,double fitness2, bool interspec_flag) {
    ...
    double p1innov = 0;  //Innovation numbers for genes inside parents' Genomes
    double p2innov = 0;
    ...
    bool disable = 0;  //Set to true if we want to disabled a chosen gene

    disable=false;
    Gene *newgene = NULL;

    bool p1better = false; //Tells if the first genome (this one) has better fitness or not
...
Genome *Genome::mate_singlepoint(Genome *g,int genomeid) {
    ...
    bool skip  = 0; //Used for skipping unwanted genes
